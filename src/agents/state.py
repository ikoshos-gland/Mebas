...
class QuestionAnalysisState(TypedDict, total=False):
    ...
    # ===== QUERY UNDERSTANDING (SOTA Pre-Retrieval) =====
    # LLM-enriched search query with curriculum terminology
    enriched_query: Optional[str]

    # Full QueryUnderstanding result from LLM
    query_understanding: Optional[dict]

    # LLM-inferred subject code (BİY, FİZ, KİM, MAT)
...
def get_effective_subject(state: QuestionAnalysisState) -> Optional[str]:
    ...
    2. inferred_subject (from LLM Query Understanding)
    3. Detected from topics (heuristic)
    4. None
    """
    if state.get("user_subject"):
        return state["user_subject"]

    # Check LLM-inferred subject (SOTA Query Understanding)
    if state.get("inferred_subject"):
        return state["inferred_subject"]

    # Try to infer from topics (fallback heuristic)
    ...
    if any(k in topics_text for k in ["fizik", "kuvvet", "hareket"]):
        return "F"
    if any(k in topics_text for k in ["biyoloji", "hücre", "canlı", "organ"]):
        return "BİY"
    if any(k in topics_text for k in ["kimya", "element", "bileşik", "reaksiyon"]):
        return "KİM"
