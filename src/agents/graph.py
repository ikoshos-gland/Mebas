...
from src.agents.nodes import (
    analyze_input,
    understand_query,  # SOTA Pre-Retrieval Query Understanding
    retrieve_kazanimlar,
    retrieve_textbook,
    rerank_results,
    track_progress,
...
def create_meb_rag_graph(checkpointer=None) -> StateGraph:
    ...
    Graph Flow (SOTA 2025 with Query Understanding):
    START → analyze_input → [message_type?]
        → CHAT (greeting/general) → handle_chat → END
        → ACADEMIC → understand_query → retrieve_kazanimlar → [success?]
    ...
    understand_query node: SOTA Pre-retrieval LLM-based query understanding.
        - Extracts key concepts from question
        - Enriches query with curriculum terminology
        - Improves retrieval accuracy by bridging semantic gap
    ...
    workflow.add_node("understand_query", understand_query)  # SOTA Query Understanding
    ...
    workflow.add_conditional_edges(
        ...
        check_analysis_success,
        {
            "continue": "understand_query",  # Academic question → Query Understanding → RAG
            "chat": "handle_chat",           # Greeting/chat → Simple response
            "error": "handle_error"
        }
    )

    # After understand_query: always go to retrieval
    workflow.add_edge("understand_query", "retrieve_kazanimlar")
